...
public class CalcRoomRent {
    ...
    public CalcRoomRent(Date mStartDate, Date mEndDate, Integer roomRent, List<Bookingaddon> mExtraBedList) {
       ...
       Bookingaddon tempextrabed;
           ...
           for(int j=0;j<mExtraBedList.size();j++)
           {
               tempextrabed = mExtraBedList.get(j);
                ...
                int extraindex = findEtraBedIndex(tempextrabed.getFromdate(),stayDays.intValue());
                if(extraindex != -1)
                {
                    insertExtraBedRate(extraindex,numDaysExtraBedUsed.intValue());
                }
    ...
    public boolean insertExtraBedRate(int extraindex,int numDaysExtraBedUsed )
    {
        DayTariff tempday ;
        for(int i =0;i<numDaysExtraBedUsed;i++)
        {
           tempday =  mTotalTarrif.get(i+extraindex);
           tempday.setExtraBedRent(mExtraRent);                    
        }
        
        return false;
    }
    public int  findEtraBedIndex(Date stDate,int stayDays)
    {
        DayTariff tempday ;
    for(int i = 0;i<stayDays;i++)
     {
         tempday = mTotalTarrif.get(i);
         if (tempday.between(stDate.getTime()) == true)
             return i;
     }
        return -1;
    }
