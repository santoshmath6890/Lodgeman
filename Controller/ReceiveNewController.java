...
import lodgeman.lalitman.controller.reports.receivereceipt;
 ...
 * @author vijay
 */
public class ReceiveNewController implements Initializable {
    ...
    public void setApp(LodgeMan aThis) 
    {
        ...
        Groups thisgroups = qm.getGroupsByOid(3);
    ...
    public void processSubmit() throws JRException, IOException
    {
        if(validate())
        {
            ...
            LodgeConfig.newReceives(paymentdatetime, selectedLedgers, note, currentUser,amount);
            /******************************************************************/
            
            /**********************************************************************/
        Receives lastreceives = qm.findLastReceives();
        Integer lastreceiveno = lastreceives.getOid();
        DirCreate dir = new DirCreate();
        Date inputP = new Date();
        String path = dir.findTargetDir(inputP);
        String path1  = path.replaceFirst("c", "C");
        String filename = path1 + "/" + "ReceiveReceipt_" + lastreceiveno + ".pdf"; 
        receivereceipt.printreceivereport(lastreceiveno, filename);
        /**********************************************************************/
